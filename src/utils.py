...
def process_body_pose_estimation(path_video, output_folder, drawing_settings):
    """
    ...
    while vidcap.isOpened():
        ...
        if len(person_boxes) > 0:
            ...
            xy = (
                torch.stack(
                    [pose_result["keypoints"] for pose_result in image_pose_result]
                )
                .cpu()
                .numpy()
            )
            scores = (
                torch.stack(
                    [pose_result["scores"] for pose_result in image_pose_result]
                )
                .cpu()
                .numpy()
            )
            ...
            # Set up annotators
            color_edge_annotator = sv.Color.from_bgr_tuple(
                drawing_settings.get("color_connections")
            )
            color_vertex_annotator = sv.Color.from_bgr_tuple(
                drawing_settings.get("color_landmarks")
            )
